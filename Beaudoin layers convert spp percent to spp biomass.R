...
start_time<-Sys.time()
for (i in 5:82){
  biomass2001[[i]]<-bs2001[[i]]*bs2001[[88]]
  biomass2011[[i]]<-bs2011[[i]]*bs2011[[88]]
}
end_time<-Sys.time()
end_time-start_time
...
for(i in 1:nlayers(biomass2001)){
  writeRaster(biomass2001[[i]], filename=paste("D:/Beaudoin/2001/Processed/sppBiomass_Canada/",names(bs2001)[i],sep=""), format="GTiff",overwrite=TRUE)
  writeRaster(biomass2011[[i]], filename=paste("D:/Beaudoin/2011/Processed/sppBiomass_Canada/",names(bs2011)[i],sep=""), format="GTiff",overwrite=TRUE)
}
end_time<-Sys.time()
end_time-start_time